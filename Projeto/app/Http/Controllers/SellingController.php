...
class SellingController extends Controller
{
    ...
    return view('tabs/vendas', [
    'categoria' => $categoria,
    'tab' => "vendas",
    'sellings' => $this->all_info(),
    'buyers' => $this->all_people(),
    'products' => $this->all_products(),
    'inputs' => $this->all_inputs(),
    'products_sellings' => $this->all_products_sellings(),
    ]);
    ...
    public function store(Request $request){
        ...
        $selling = new Selling;
        $selling->person_id = $request->supplier;
        $selling->date = $request->date;
        $selling->delivery = $request->delivery;
        $selling->discount = $request->discount;
        $selling->situation = $request->situation;
        $selling->observations = $request->observations;
        $selling->price = $request->price;
        $selling->save();

        for ($i=0; $i < sizeof($input["input"]["nome"]); $i++) { 

            if($this->get_product_by_name($input["input"]["nome"][$i]) == NULL){
                DB::table('products')
                ->insert([
                    'name' => $input["input"]["nome"][$i],
                    'price' => $input["input"]["valor-unidade"][$i],
            ...
            DB::table('product_selling')
            ->insert([
                'selling_id' => $selling->id,
                'product_id' => $this->get_product_by_name($input["input"]["nome"][$i]) != NULL ? $this->get_product_by_name($input["input"]["nome"][$i])->id : NULL,
    ...
    public function update(Request $request){
        ...
        Selling::findOrFail($request->id)->update(
            ['date' => $request->date,
            ...
            return redirect('/vendas')->with('msg', 'Venda alterada com sucesso!');
    }

    public function destroy($id){
        DB::table('product_selling')->where('selling_id', $id)->delete();
        DB::table('sellings')->where('id', $id)->delete();
        return redirect('/vendas')->with('msg', 'Venda excluída com sucesso!');
    }
    ...
    private function all_products_sellings(){
        $product_selling = DB::table('product_selling')   
        ->get();
    ...
    public function table(){
        ...
        if($search == "filter_off"){
            $buys = DB::table('sellings')
                            ->latest('sellings.updated_at')
                            ->get();
        } else {
            $buys = Selling::where([['id', 'like', '%'.$search.'%']])
                                    ->latest('sellings.updated_at')
                                    ->get();
        }
        ...
        $html .= "
        <tr>
        <td data-bs-toggle='modal' data-bs-target='#selling-id-$buy->id'>$buy->id</td>
        <td data-bs-toggle='modal' data-bs-target='#selling-id-$buy->id'>$supplier->name</td>
        <td data-bs-toggle='modal' data-bs-target='#selling-id-$buy->id'>$contact->smartphone</td>
        <td data-bs-toggle='modal' data-bs-target='#selling-id-$buy->id'>";
        $html .= date('d/m/Y', strtotime($buy->date));

        $html .= "<td data-bs-toggle='modal' data-bs-target='#selling-id-$buy->id'>";
        $html .= date('d/m/Y', strtotime($buy->delivery));


        $html .= "</td>
        <td data-bs-toggle='modal' data-bs-target='#sellingid-$buy->id'>R$ $buy->price</td>
